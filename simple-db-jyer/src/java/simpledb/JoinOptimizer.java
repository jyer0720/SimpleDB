...
public class JoinOptimizer {
    ...
    public <T> Iterator<Set<T>> subsetsIterator(Vector<T> v, int size) {
    	BitsetIterator bitIte = new BitsetIterator(v.size(), size);
    	return new Iterator<Set<T>>() {
    		
    		public boolean hasNext() {
    			return bitIte.hasNext();
    		}
    		
    		public Set<T> next() {
    			Set<T> res = new HashSet<>();
    			List<Integer> bitList = bitIte.next();
    ...
    public class BitsetIterator implements Iterator<List<Integer>>{
    	
    	private final int size;
    	private final int subSize;
    	private List<Integer> next;
    	
    	public BitsetIterator(int size, int subSize) {
    		this.size = size;
    		this.subSize = subSize;
    		next = new ArrayList<>();
    		for (int i = 0; i < size; i++) {
    			if (i < subSize) next.add(1);
    			else next.add(0);
    		}
    	}
    	...
    	public List<Integer> next() {
    		...
    		if (next.get(size-1) == 0) {
    			...
    			int count = 1;
    			int i = size - 1;
    			while (i >= 0 && next.get(i) == 1) {
    				next.set(i, 0);
    				count++;
    				i--;
    			}
    			while (i >= 0 && next.get(i) == 0)
    				i--;
    			if (i < 0) {
    				next = null;
    				return res;
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	for (int i = 1; i <= joins.size(); i++) {
    		Iterator<Set<LogicalJoinNode>> ite = subsetsIterator(joins, i);
    		while (ite.hasNext()) {
    		//Set<Set<LogicalJoinNode>> sets = enumerateSubsets(joins, i);
    		//for (Set<LogicalJoinNode> set : sets) {
    			Set<LogicalJoinNode> set = ite.next();
    			double bestCostSoFar = Double.MAX_VALUE;
    			CostCard cc = null;
    			for (LogicalJoinNode lj : set) {
    				CostCard tmpCc = computeCostAndCardOfSubplan(stats, filterSelectivities, 
    						lj, set, bestCostSoFar, pCache);
    				if (tmpCc != null && tmpCc.cost < bestCostSoFar) {
    					cc = tmpCc;
    					bestCostSoFar = cc.cost;
    				}
    			}
    			if (cc != null)
    				pCache.addPlan(set, cc.cost, cc.card, cc.plan);
    		}
    	}
    	Vector<LogicalJoinNode> res = pCache.getOrder(new HashSet<>(joins));
    	if (explain) {
    		printJoins(res, pCache, stats, filterSelectivities);
    	}
