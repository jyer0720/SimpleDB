        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                if (!tidToFirstLogRecord.containsKey(tid.getId())) {
                	throw new NoSuchElementException("tid not found");
                }
                long firstOffset = tidToFirstLogRecord.get(tid.getId());
                long lastOffset = raf.getFilePointer();
                long tempOffset = currentOffset;
                while(tempOffset > firstOffset) {
                	tempOffset -= LONG_SIZE;
                	raf.seek(tempOffset);
                	tempOffset = raf.readLong();
                	raf.seek(tempOffset);
                	int rType = raf.readInt();
                	long rTid = raf.readLong();
                	if (rTid == tid.getId() && rType == UPDATE_RECORD) {
                		Page page = readPageData(raf);
                		Database.getCatalog().getDatabaseFile(page.getId().
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                raf.seek(raf.length());
                Set<Long> completedTid = new HashSet<>();
                long currOffset = raf.length();
                while (currOffset > LONG_SIZE) {
                	currOffset -= LONG_SIZE;
                	raf.seek(currOffset);
                	currOffset = raf.readLong();
                	raf.seek(currOffset);
                	int type = raf.readInt();
                	long rTid = raf.readLong();
                	if (type == COMMIT_RECORD || type == ABORT_RECORD) {
                		completedTid.add(rTid);
                	}
                	if (type == UPDATE_RECORD) {
                		Page beforeImage = readPageData(raf);
                		//Page afterImage = readPageData(raf);
                		if (!completedTid.contains(rTid)) {
                			Database.getCatalog().getDatabaseFile(beforeImage.
                					getId().getTableId()).writePage(beforeImage);
                		}
                	}
                	raf.seek(raf.length());
